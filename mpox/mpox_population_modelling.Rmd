...
for(iter in 243:1000) {
  ...
  for (k in 1:K) {
    ...
    if(pgamma(alpha_k_minus, a_k, b_k) == pgamma(alpha_k_plus, a_k, b_k)) {
      p_minus = mpfr(pgamma(alpha_k_minus, a_k, b_k, log.p = TRUE), 300)
      p_plus = mpfr(pgamma(alpha_k_plus, a_k, b_k, log.p = TRUE), 300)
      alpha_samples[iter, k] = qgamma(as.numeric(log(runif(1) * (exp(p_plus) - exp(p_minus)) + exp(p_minus))), a_k, b_k, log.p = TRUE)
    } else {
