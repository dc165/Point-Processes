...
for(iter in 537:1000) {
  ...
  for (k in 1:K) {
    ...
    if(pgamma(alpha_k_minus, a_k, b_k) == pgamma(alpha_k_plus, a_k, b_k)) {
      p_minus = pgamma(alpha_k_minus, a_k, b_k, log.p = TRUE)
      p_plus = pgamma(alpha_k_plus, a_k, b_k, log.p = TRUE)
      if(p_minus != -Inf) {
        p_minus = mpfr(p_minus, ceiling(abs(log(abs(p_minus), 2))) + 64)
      }
      if(p_plus != 0) {
        p_plus = mpfr(p_plus, ceiling(abs(log(abs(p_plus), 2))) + 64)
      }
      alpha_samples[iter, k] = qgamma(as.numeric(log(1 - exp(p_minus) - runif(1) * (exp(p_plus) - exp(p_minus)))), a_k, b_k, log.p = TRUE, lower.tail = FALSE)
    } else {
  ...
  for(root in as.numeric(names(table(roots)))) {
    ...
    t_idx = ifelse(t_idx < t_init, t_init, t_idx)
    # Burn in observation doesn't have a likelihood
    log_prob = ifelse(root >= n_init, log(mu_samples[iter,]), 1)
    log_prob = log_prob - alpha_samples[iter, ] * sum(D_j * sweep(exp(-t_idx %o% beta_samples[iter, ]), 2, exp(-beta_samples[iter, ] * T)))
    if(length(O) > 0){
      t_j = data$onset[O]
      t_i = data$onset[B_samples[iter - 1, O]]
      z_j = z_samples[iter - 1, O]
...
```{r}
load(file.path("MCMC outputs", paste("model", 2), "mu_samples.RData"))
load(file.path("MCMC outputs", paste("model", 2), "alpha_samples.RData"))
load(file.path("MCMC outputs", paste("model", 2), "beta_samples.RData"))
load(file.path("MCMC outputs", paste("model", 2), "sigma_samples.RData"))
load(file.path("MCMC outputs", paste("model", 2), "B_samples.RData"))
load(file.path("MCMC outputs", paste("model", 2), "kappa_samples.RData"))
load(file.path("MCMC outputs", paste("model", 2), "z_samples.RData"))
load(file.path("MCMC outputs", paste("model", 2), "theta_samples.RData"))
```
...
```
```{r}
plot(alpha_samples[20:1000, 1], sigma_samples[20:1000, 1])
```

```{r}
kappa_props = rowSums(kappa_samples[20:1000,] - 1)/N
hist(kappa_props, breaks = 150, prob = TRUE, main = "Density of clade 2 proportions", xlab = "Proportion of total events classified as clade 2")
```
```{r}
props = kappa_props <= 0.1
props = props + ((kappa_props > 0.1) & (kappa_props <= 0.5)) * 2
props = props + (kappa_props > 0.5) * 3
pairs(param_samples, col = c("red", "black", "green")[props])
```

```{r}
hist(theta_samples[200:1000, 141, 2], breaks = 300)
```
...
```
```{r}
colnames(param_samples) = c("mu 1", "mu 2", "alpha 1", "alpha 2", "beta 1", "beta 2", "sigma 1", "sigma 2")
scatterplotMatrix(param_samples)
```


```{r}
param_samples = cbind(mu_samples[20:1000, ], 
                      alpha_samples[20:1000, ],
                      beta_samples[20:1000, ],
                      sigma_samples[20:1000, ],
                      kappa_props)
# transform = rbind(colMeans(param_samples), apply(param_samples, MARGIN = 2, sd))
# for(col in 1:9){
#   param_samples[,col] = (param_samples[,col] - transform[1, col])/transform[2, col]
# }
```

```{r}
library(meanShiftR)
posterior_modes = meanShift(param_samples, algorithm = "KDTREE", iterations = 1000, bandwidth = rep(10000, 9))

modes_idx = as.numeric(names(sort(table(posterior_modes$assignment), decreasing = TRUE)[1:10]))
modes_value = posterior_modes$value[modes_idx,]
modes_value
```
```{r}
mu = modes_value[1, 1:2]
alpha = modes_value[1, 3:4]
beta = modes_value[1, 5:6]
sigma = modes_value[1, 7:8]
cell_densities = sum(density ~ cell, data = data.frame(coords))
...
for(i in 1:1) {
  ...
  while(t_max < 1) {
    ...
    if(t < 1) {
      nodes = c(nodes, t)
      locs = c(locs, sample(coords[,"idx"], 1))
      t_max = t
      E_count_mu[k] = E_count_mu[k] + 1
    }
  }
  while(length(nodes) > 0) {
    ...
    nodes = nodes[-1]
    locs = locs[-1]
    lambda_max = alpha[k] * coords[z_est, "density"] * beta[k]
    accept = FALSE
    while(!accept) {
      t_prop = rexp(1, lambda_max)
      lambda_t = lambda_max * exp(-beta[k] * t_prop)
      if(runif(1) < lambda_t/lambda_max) {
        accept = TRUE
        if(parent + t_prop < 1) {
          E_count_mu[k] = E_count_mu[k] + 1
          nodes = c(nodes, t_prop)
          nodes = c(nodes, t_prop)
          locs = c(locs, z_est)
          prob = coords[,"density"] * exp(-rowSums(sweep(coords[,1:2], coords[z_est, 1:2])^2)/(2 * sigma[k]))
          locs = c(locs, sample( , 1))
        }
      }
    }
...
for(day in t_init:T) {
  obs_idx = which((data$onset < day) & (data$onset >= (day - 14)))
  t_obs = data$onset[obs_idx]
  kappa = kappa_samples[200:1000, obs_idx]
  z = z_samples[200:1000, obs_idx]
  E_count = 0
  for(j in 1:801) {
    kappa_est = kappa[j, ]
    z_est = z_samples[j, ]
    D_est = coords[z_est, 4]
    alpha_est = alpha[kappa_est]
    beta_est = beta[kappa_est]
